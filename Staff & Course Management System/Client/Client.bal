...
public function main() returns error? {
    ...
    io:println("-------------------------------------------------------------------------------");
    io:println("|                                                                             |");
    io:println("|                       Ultimate Management System v5.4.0                     |");
    io:println("|                                                                             |");
    io:println("-------------------------------------------------------------------------------");
...
public function Get_all_courses_by_NQFlevel_function(http:Client http, string NQF_level) returns error? {
    if (http is http:Client) {
        Course[] course = check http->/getCoursesbyNQFlevel(NQF_level = NQF_level);
        foreach Lecturers item in course{
        io:println("--------------------------");
        io:println("This Course Has The Following NQF Level: ", item.NQF_level);
        io:println("Course Code: ", item.Course_code);
